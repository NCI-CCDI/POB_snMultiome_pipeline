...
def copyWebSummary():
    ...
    for filename in files:
        ...
        if sample_name != "AggregatedDatasets":
            gex_summary = glob.glob(f"./{sample_name}/SC_ATAC_GEX_COUNTER_CS/SC_ATAC_GEX_COUNTER/GEX_SUMMARIZE_REPORTS/*/*/files/web_summary.html")
            copyfile(gex_summary[0], '%s/%s_GEX_web_summary.html' % (summaryPath, sample_name))
            atac_summary = glob.glob(f"./{sample_name}/SC_ATAC_GEX_COUNTER_CS/SC_ATAC_GEX_COUNTER/_SC_ATAC_REPORTER/CREATE_WEBSUMMARY/*/*/files/web_summary.html") 
