...
df_threshold = data.frame("Sample" = character(), "nCount_RNA_LL" = numeric(), "nCount_RNA_UL" = numeric(), "nFeature_RNA_LL" = numeric(), "mt_UL" = numeric(),
                         ...
                         "pct_fragments_in_peaks_LL"= numeric(), "blacklist_fraction_LL"= numeric(),"doublet_score_cutoff"=numeric(),"doublet_score_percentile"=numeric(),
...
df_filter = data.frame("Sample" = character(), "nCount_RNA_LL"=numeric(), "nCount_RNA_UL"=numeric(),"nFeature_RNA_LL"=numeric(),"percent_mt"=numeric(),
                       "nCount_ATAC_LL"=numeric(),"nCount_ATAC_UL"=numeric(),"nFeature_ATAC"=numeric(),
                       "nucleosome_signal"=numeric(), "TSSenrichment"=numeric(),
                       "pct_fragments_in_peaks"=numeric(), "blacklist_fraction"=numeric(),
                       "total_filt"=numeric(), "scrablet"=numeric(),"total_with_doublet_filter"=numeric(),"scrublet_percentile"=numeric(),"total_with_doublet_filter_percentile"=numeric(),
                       "total"=numeric(),
                       stringsAsFactors = FALSE)

for (sample in samplelist) {
  #sample = "CCDI0270_TCU00389-0201-Rep1"
  print(sample)
  ...
  thresholdStat = c(sample,nCount_RNA_LL,nCount_RNA_UL,nFeature_RNA_LL,mt_UL,
                ...
                nucleosome_signal_UL,TSSenrichment_LL,pct_reads_in_peaks_LL,blacklist_fraction_UL,doublet_threshold)
  ...
  colnames(df_threshold_now) = c("Sample", "nCount_RNA_LL","nCount_RNA_UL", "nFeature_RNA_LL", "mt_UL",
                       ...
                       "pct_fragments_in_peaks_LL", "blacklist_fraction_LL","doublet_score_percentile")
  ...
  nucleasome_UL_filt = sum(m$nucleosome_signal >= nucleosome_signal_UL)
  ...
  colnames(df_filt_now) = data.frame("Sample", "nCount_RNA_LL", "nCount_RNA_UL","nFeature_RNA","percent_mt",
                         "nCount_ATAC_LL","nCount_ATAC_UL","ATAC_nFeature",
                         "nucleosome_signal", "TSSenrichment",
                         "pct_fragments_in_peaks", "blacklist_fraction",
                         "total_filt","scrublet","total_with_doublet_filter","scrublet_","total_with_doublet_filter_percentile","total")
