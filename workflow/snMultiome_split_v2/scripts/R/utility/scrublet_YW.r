...
scrublet_YW <- function(object, min_counts=2, min_cells=3, expected_doublet_rate=0.06, doublet_rate_adj=0.05, min_gene_variability_pctl=85, n_prin_comps=30, sim_doublet_ratio=2,sample="ccdi",path="/data/CCRCCDI", umi_min = 0, ngene_min= 0, mt_max = 100,n_neighbors=NULL) {
    #umi_min = 500
    #ngene_min = 300
    #mt_max = 10
    ...
    cat("filter umi:", umi_min,"\n",
        ...
        "filter mt:", mt_max, "\n")
    cat("expected_doublet_rate:",expected_doublet_rate,"\n",
    ...
    #reticulate::source_python('/data/CCRCCDI/software/Scripts/current_snakemake/snMultiome_split_v2/scripts/R/utility/scrublet_py_YW_v2.py')
    reticulate::source_python(paste0(current_dir,'/scripts/R/utility/scrublet_py_YW.py'))
    ...
    scrublet_py_args <- c(
      list(
        ...
        expected_doublet_rate=doublet_rate, doublet_rate_adj=doublet_rate_adj,  min_counts=2, 
    ...
    #(doublet_scores, predicted_doublets,cutoff_auto,cutoff_gmm1, cutoff_percentile1,cutoff_final,scr_cri,percentile,cutoff_percentile1_adj,percentile_adj)
    names(scrublet_res) <- c('doublet_scores', 'scrub.auto_doublet','scrub.cutoff_auto','scrub.cutoff_gmm','scrub.cutoff_percentile','scrub.cutoff_final','scrub.criteria','scrub.percentile','scrub.cutoff_percentile_adj','scrub.percentile_adj')
    #print(scrublet_res)
    ...
    object_ori@meta.data <- object_ori@meta.data %>%
      ...
      mutate(scrub.if_doublet_percentile_adj = ifelse(is.na(doublet_scores), "Low_quality", 
                                                      ifelse(doublet_scores < scrublet_res$scrub.cutoff_percentile_adj, "Singlet", "Doublet"))) %>%
