...
def fit_Gaussian(data, mu1, sigma1, mu2, sigma2,doublet_rate,doublet_scores,cutoff_auto,sample):
    # Generate x values
    ...
    while attempt <= max_attempts:
        try:
            ...
            # Handle the error (e.g., adjust min_val, max_val, or retry)
            # For example:
            #min_val *= 0.9  # Adjust min_val
            #max_val *= 1.1  # Adjust max_val
            attempt += 1    # Increment the attempt counter
    else:
        # This block executes if the loop completes without a successful attempt
        print("Maximum attempts reached. Unable to find intersection.")
    ...
    plt.axvline(percentile_score, color="red",linestyle='--', label=f'{percentile}th Percentile_{percentile_score}\n(Doublet:{len(filter2)})')
    ...
    #plt.show()
    intersection_val =  round(intersection.root,3)
    percentile_cutoff = round(percentile_score,3)
    print("round to 3 decimal")
    print(intersection.root, intersection_val, percentile_score, percentile_cutoff)
    return intersection_val, percentile_cutoff,percentile, attempt, plt
...
def scrublet_py_YW(i, j, val, dim, expected_doublet_rate, min_counts, min_cells, min_gene_variability_pctl, n_prin_comps, sim_doublet_ratio, n_neighbors,sample,path):
    ...
    print(attempt)
    print(mean1,sigma1, mean2, sigma2)
    cutoff_gmm1, cutoff_percentile1,percentile,attempt2,plot1 = fit_Gaussian (score_data,mean1,sigma1,mean2,sigma2,doublet_rate,doublet_scores,cutoff_auto,sample)
    ...
    scr_cri = "gmm"
    if (attempt > 1 | attempt2 > 1):
      cat("attempt fit:", attempt, "\n", "attempt intersection:",attempt2,"\n")
      cutoff_final = cutoff_percentile1
      scr_cri = "percentile"
    print ("auto,gmm,percentile")
    print (cutoff_auto,cutoff_gmm1, cutoff_percentile1)
    print (attempt,cutoff_final,scr_cri)
    return(doublet_scores, predicted_doublets,cutoff_auto,cutoff_gmm1, cutoff_percentile1,cutoff_final,scr_cri,percentile)
